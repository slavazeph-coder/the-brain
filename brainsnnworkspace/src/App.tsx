...
export default function App() {
  ...
  // Google Drive & Export Hub States
  ...
  const [driveStatusType, setDriveStatusType] = useState<'info' | 'success' | 'error'>('info');
  const [showDriveConfig, setShowDriveConfig] = useState(false);
  const [clientIdInput, setClientIdInput] = useState("102934812349-demoapps.apps.googleusercontent.com"); // default demo/instruction key

  // Listen for Google OAuth callback from the popup window
  useEffect(() => {
    const handleOAuthMessage = (event: MessageEvent) => {
      const origin = event.origin;
      // Accept local development and standard Cloud Run domains
      if (!origin.endsWith('.run.app') && !origin.includes('localhost') && !origin.includes('.aistudio.google')) {
        return;
      }

      if (event.data?.type === 'OAUTH_AUTH_SUCCESS') {
        console.log("OAuth Authentication success payload received from popup:", event.data);
        if (event.data.accessToken) {
          setGoogleAccessToken(event.data.accessToken);
          setDriveMessage("Connected successfully! Ready to export workspace to Google Drive.");
          setDriveStatusType("success");
        } else {
          setDriveMessage("Authenticated, but no Access Token found in redirection fragment.");
          setDriveStatusType("error");
        }
      }
    };
  ...
  const startGoogleAuth = () => {
    setDriveMessage("Opening secure Google OAuth consent popup...");
    setDriveStatusType("info");
    ...
    const params = new URLSearchParams({
      client_id: clientIdInput.trim() || "102934812349-demoapps.apps.googleusercontent.com",
    ...
    if (!authWindow) {
      setDriveMessage("⚠️ Popup blocked! Please allow popups for this site to complete authorization.");
      setDriveStatusType("error");
    }
  };
  ...
  const exportToGoogleDrive = async () => {
    ...
    setDriveMessage("Generating zipped deployment codebase...");
    setDriveStatusType("info");

    try {
      // 1. Fetch the project zip blob from the server
      const zipRes = await fetch('/api/download-zip');
      if (!zipRes.ok) {
        throw new Error(`Failed to generate ZIP on the backend (Status ${zipRes.status})`);
      }
      const zipBlob = await zipRes.blob();
      
      setDriveMessage("Connected to Google Drive API. Creating folder 'BrainSNN-Workspace'...");
      
      // 2. Create target folder on Google Drive
      const folderMetadata = {
        name: "BrainSNN Workspace Exports",
      ...
      if (!folderRes.ok) {
        const errDetails = await folderRes.json().catch(() => ({}));
        throw new Error(`Folder creation failed: ${errDetails.error?.message || folderRes.statusText}`);
      }
      ...
      setDriveMessage("Folder created! Uploading 'brainsnn-workspace.zip' into folder...");

      // 3. Upload Zip File to the Folder
      const fileMetadata = {
        name: "brainsnn-project-source.zip",
        parents: [folderId],
      };
      ...
      if (!uploadRes.ok) {
        const errDetails = await uploadRes.json().catch(() => ({}));
        throw new Error(`File upload failed: ${errDetails.error?.message || uploadRes.statusText}`);
      }

      const fileData = await uploadRes.json();
      setDriveFileId(fileData.id);

      setDriveMessage("🎉 Success! Complete workspace successfully zipped and uploaded to your Google Drive!");
      setDriveStatusType("success");
    } catch (err: any) {
      console.error("Google Drive upload error:", err);
      setDriveMessage(`Upload failed: ${err.message || err}`);
      setDriveStatusType("error");
    } finally {
  ...
  return (
    <div className="w-full min-h-screen bg-[#0a0a0f] text-[#e0e0e0] flex flex-col font-sans selection:bg-cyan-500/30 selection:text-cyan-200">
      ...
      {/* GLOBAL HIGH-VISIBILITY DEPLOYMENT EXPORTER BANNER */}
      <div className="w-full bg-[#1e1b4b] border-b border-indigo-500/30 text-white py-2.5 px-4 font-sans text-xs flex flex-col sm:flex-row gap-3 items-center justify-between z-[60] relative shadow-[0_4px_20px_rgba(99,102,241,0.15)]">
        <div className="flex items-center gap-2.5">
          <span className="flex h-2 w-2 relative">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
          </span>
          <span className="text-indigo-200 font-semibold uppercase tracking-wider font-mono text-[9px] bg-indigo-950/80 px-2 py-0.5 rounded border border-indigo-400/20">READY TO INTEGRATE</span>
          <p className="text-zinc-200">
            BrainSNN full source files are ready for packaging. You can download the full workspace ZIP locally or save.
          </p>
        </div>
        <div className="flex items-center gap-2.5 shrink-0">
          {/* Quick Instant direct ZIP download link */}
          <a
            href="/api/download-zip"
            className="px-3.5 py-1.5 bg-cyan-500 hover:bg-cyan-400 text-black font-black uppercase text-[10px] rounded hover:scale-105 transition-all flex items-center gap-1.5 shadow-[0_0_12px_rgba(6,182,212,0.4)]"
          >
            <Download className="w-3.5 h-3.5" />
            <span>Download ZIP</span>
          </a>
          
          {/* Google Drive automatic trigger */}
          <button
            onClick={() => {
              setActiveTab('lab');
              setTimeout(() => {
                const target = document.getElementById("active-tab-container-anchor");
                if (target) {
                  target.scrollIntoView({ behavior: 'smooth' });
                } else {
            ...
            className="px-3.5 py-1.5 bg-[#a855f7] hover:bg-purple-600 text-white font-bold uppercase text-[10px] rounded hover:scale-105 transition-all flex items-center gap-1.5 shadow-[0_0_12px_rgba(168,85,247,0.4)] cursor-pointer"
          >
      ...
      <main className="max-w-7xl mx-auto w-full px-4 md:px-8 py-10 z-10 flex-1 flex flex-col gap-10" id="main-content-layout">
        ...
        {activeTab === 'lab' && (
          <div className="space-y-6 animate-fadeIn" id="active-tab-container-anchor">
            ...
            {/* SNN WORKSPACE CODE EXPORTER AND GOOGLE DRIVE ARCHIVER */}
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 bg-[#0c0c14] border border-[#a855f7]/20 p-6 rounded-2xl relative overflow-hidden shadow-[0_4px_30px_rgba(168,85,247,0.05)]">
              {/* background atmospheric glow */}
              <div className="absolute top-0 right-0 w-64 h-64 bg-cyan-500/5 rounded-full blur-3xl pointer-events-none"></div>
              
              <div className="md:col-span-7 space-y-4">
                <div className="inline-flex items-center gap-2 px-2.5 py-1 bg-cyan-950/30 border border-cyan-500/20 rounded-full text-[10px] font-bold text-cyan-400 uppercase tracking-widest font-mono">
                  <Layers className="w-3.5 h-3.5" />
                  <span>Deployment Archiver & Google Drive Sync</span>
                </div>
                
                <h4 className="text-lg font-bold text-white font-sans">
                  Export complete BrainSNN source code codebase
                </h4>
                
                <p className="text-xs text-zinc-400 leading-relaxed font-sans">
                  Download the clean development workspace containing the React/TypeScript frontend and Express server as a structured, ready-to-run ZIP package. You can also deploy this archive directly into an organized Google Drive folder for cloud-hosting and persistence.
                </p>

                {/* status message */}
                {driveMessage && (
                  <div className={`p-3.5 rounded-xl border flex items-start gap-2.5 font-sans text-xs ${
                    driveStatusType === 'success' 
                      ? "bg-emerald-950/20 border-emerald-500/20 text-emerald-300"
                      : driveStatusType === 'error'
                      ? "bg-red-950/20 border-red-500/20 text-red-300"
                      : "bg-[#0f172a] border-cyan-500/20 text-cyan-300"
                  }`}>
                    {driveStatusType === 'success' ? (
                      ...
                      <div className="w-4.5 h-4.5 border-2 border-cyan-500 border-t-transparent rounded-full animate-spin shrink-0 mt-0.5 font-mono"></div>
                    )}
                    <div className="space-y-1">
                      <p>{driveMessage}</p>
                      {driveFolderId && (
                        <p className="text-[10px] font-mono mt-1 opacity-80">
                          Folder ID: <span className="bg-zinc-950 px-1.5 py-0.5 rounded">{driveFolderId}</span>
                        </p>
                      )}
                      {driveFileId && (
                        <p className="text-[10px] font-mono mt-1">
                          <a 
                            ...
                            className="text-[#00f5ff] underline inline-flex items-center gap-1 hover:text-white"
                          >
                            Open Zipped Code on Google Drive <ArrowUpRight className="w-3 h-3" />
                          </a>
                        </p>
                      )}
                    </div>
                ...
                {/* Collapsible API configuration client ID */}
                <div className="border border-white/5 rounded-xl overflow-hidden bg-black/20 text-xs">
                  <button 
                    ...
                    className="w-full px-4 py-2.5 flex items-center justify-between text-zinc-400 hover:text-white font-mono uppercase tracking-wider text-[10px] cursor-pointer"
                  >
                    <span>🛠️ Google API Client Configuration</span>
                    <span>{showDriveConfig ? "Collapse" : "Expand"}</span>
                  </button>
                  {showDriveConfig && (
                    <div className="p-4 border-t border-white/5 space-y-3 font-sans text-xs text-zinc-400">
                      <p>
                        To upload files securely from your browser to your Google Drive, we use the secure <strong>Implicit OAuth 2.0 Flow</strong>. This allows you to log in directly through Google with your own developer client ID.
                      </p>
                      <div className="space-y-1">
                        <label className="block text-[10px] font-mono text-zinc-500 uppercase">Google Client ID</label>
                        <input 
                          type="text" 
                          value={clientIdInput}
                          onChange={(e) => setClientIdInput(e.target.value)}
                          placeholder="Your Client ID from GCP Console"
                          className="w-full bg-[#06060a] border border-white/10 px-3 py-2 rounded text-white font-mono text-xs focus:border-[#a855f7] outline-none"
                        />
                      </div>
                      <p className="text-[10px] text-zinc-500 italic">
                        Tip: You can use your own Client ID generated at <a href="https://console.cloud.google.com/apis/credentials" target="_blank" rel="noopener noreferrer" className="text-cyan-400 underline">GCP Credentials</a>. Ensure <code>{window.location.origin}/auth/callback</code> is added as an Authorized Redirect URI.
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="md:col-span-5 flex flex-col justify-center gap-3">
                {/* Direct download code ZIP */}
                <a 
                  href="/api/download-zip"
                  className="w-full flex items-center justify-center gap-3 px-5 py-3.5 bg-gradient-to-r from-cyan-950/20 to-indigo-950/20 border border-cyan-500/30 hover:border-cyan-400 hover:text-white text-cyan-200 font-bold font-sans text-xs uppercase tracking-wider rounded-xl transition-all hover:bg-cyan-950/50 cursor-pointer shadow-[0_0_15px_rgba(6,182,212,0.1)] text-center font-bold"
                >
                  <Download className="w-4.5 h-4.5 text-cyan-400 shrink-0" />
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-3 px-5 py-3.5 bg-[#a855f7] hover:bg-purple-600 disabled:bg-purple-950/40 text-white font-bold font-sans text-xs uppercase tracking-wider rounded-xl transition-all shadow-[0_0_20px_rgba(168,85,247,0.25)] hover:shadow-[0_0_30px_rgba(168,85,247,0.4)] disabled:shadow-none cursor-pointer disabled:cursor-not-allowed text-center"
                >
                  {driveExporting ? (
                    ...
                    <Cloud className="w-4.5 h-4.5 text-purple-200" />
                  )}
                  <span>
                    {driveExporting 
                      ? "Exporting Code..." 
                ...
                {googleAccessToken && (
                  <button 
                    onClick={() => {
                      ...
                      setDriveMessage("Google tokens cleared from active session memory.");
                      setDriveStatusType("info");
                    }} 
                    className="text-center font-mono text-[9px] text-zinc-500 uppercase tracking-widest hover:text-red-400 mt-1 cursor-pointer"
                  >
