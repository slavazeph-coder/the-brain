...
test('ambiguous payment-service failure retains its hold and idempotency key',async()=>{
 let keys=[],bad=true;const fake={checkout:{sessions:{create:async(p,o)=>{keys.push(o.idempotencyKey);if(bad)throw new Error('timeout');return{id:'cs_test_retry',url:'https://checkout.stripe.com/c/pay/cs_test_retry',expires_at:Math.floor(Date.now()/1000)+1800};}}}};
 const f=fixture({getStripe:()=>fake});try{const id=crypto.randomUUID();await assert.rejects(f.direct.purchase(valid(),id),/retained safely/);assert.equal(f.d.prepare('SELECT reserved_slot FROM sponsor_applications').get().reserved_slot,'001:chest');bad=false;await f.direct.purchase(valid(),id);assert.equal(keys.length,2);assert.equal(keys[0],keys[1]);}finally{f.close();}
});
