...
try{
 ...
 const ownerContext=await browser.newContext({viewport:{width:1200,height:900}});const owner=await ownerContext.newPage();await owner.goto(base+'/sponsor/admin/');await owner.locator('#owner-login:not([hidden])').waitFor();await owner.locator('[name=password]').fill('browser-test-only-owner-key-at-least-32');await owner.locator('#owner-login button').click();await owner.locator('#owner-workspace:not([hidden])').waitFor();await owner.getByRole('heading',{name:'Isolated Example Test'}).waitFor();evidence.checks.push('Owner authenticated and reviewed the actual saved application.');await owner.screenshot({path:path.join(out,'owner-dashboard.png'),fullPage:true});evidence.screenshots.push('owner-dashboard.png');await ownerContext.close();
