...
let uploadVersion=0,editTimer=0,uploading=false;
const anchors={
...
el('logo').addEventListener('change',async e=>{const file=e.target.files?.[0],v=++uploadVersion;if(!file)return;uploading=true;document.dispatchEvent(new CustomEvent('gt3:design',{detail:{pending:true}}));let url;try{
 ...
 }catch(err){artStatus(err.message);}finally{if(url)URL.revokeObjectURL(url);e.target.value='';if(v===uploadVersion){uploading=false;document.dispatchEvent(new CustomEvent('gt3:design',{detail:{placed:!!artwork.decal,zone:artwork.zone}}));}}});
el('remove-logo').addEventListener('click',()=>{uploadVersion++;uploading=false;removeDecal();artwork.image=null;artwork.brand='';artwork.texture?.dispose();artwork.texture=null;el('brand-name').value='';el('remove-logo').disabled=true;el('upload-label').textContent='Upload your logo';artStatus('Preview cleared. Choose another logo or type your brand.');document.dispatchEvent(new CustomEvent('gt3:design',{detail:{placed:false}}));});
