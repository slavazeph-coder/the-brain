'use strict';
// Isolated, non-binding GT3 intake. This module never creates payments or robot reservations.
...
function createHandler(options={}){
 ...
 async function body(req){if(!String(req.headers['content-type']).includes('application/json'))fail(415,'JSON required.');let size=0;const chunks=[];for await(const c of req){size+=c.length;if(size>16384)fail(413,'Proposal is too large.');chunks.push(c);}try{return JSON.parse(Buffer.concat(chunks).toString());}catch{fail(400,'Invalid JSON.');}}
 async function handle(req,res){
  try{
   ...
   if(p===API+'/session'&&req.method==='GET'){
    ...
    return json(res,200,{csrf:mac('csrf:'+value),applications:'accepting',payments:'disabled',terms:VERSION});
   }
   if(p===API+'/status'&&req.method==='GET'){database();return json(res,200,{ok:true,storage:'persistent_sqlite',applications:'accepting',payments:'disabled',version:VERSION});}
   ...
   const allowed=new Set(['index.html','app.js','style.css','model.js','engine.js','rear.webp','side.webp','admin.html','admin.js']);
   if(!allowed.has(name))fail(404,'Page not found.');
   res.setHeader('Content-Security-Policy',"default-src 'none'; script-src 'self' https://static.sketchfab.com; style-src 'self'; img-src 'self' data: blob:; frame-src https://sketchfab.com; connect-src 'self'; font-src 'self'; object-src 'none'; base-uri 'none'; form-action 'self'; frame-ancestors 'none'");
   if(name==='admin.html')res.setHeader('X-Robots-Tag','noindex, nofollow');
   const file=path.join(publicDir,name),data=fs.readFileSync(file),mime={'.html':'text/html; charset=utf-8','.js':'text/javascript; charset=utf-8','.css':'text/css; charset=utf-8','.webp':'image/webp'};
 ...
 return {handle,database,close(){db?.close();db=undefined;},matches:p=>/^\/(?:sponsor\/gt3(?:\/|$)|api\/gt3(?:\/|$))/.test(p.split('?')[0])};
}
function register(){
