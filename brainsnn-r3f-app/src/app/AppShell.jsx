...
export function AppShell() {
  ...
  if(route==='home')return <BehaviourHome/>; if(route==='missions')return <ProofMissionsPage/>; if(route==='survival')return <SurvivalWorldPage/>; if(route==='lab')return <React.Suspense fallback={<div className="powder-boot">Loading the lab…</div>}><PowderLabPage/></React.Suspense>; if(route==='reconstruct')return <ReconstructPage onHome={openLanding} onStart={openWorkspace}/>; if(route==='evidence')return <HoldoutEvidencePage onHome={openLanding} onStart={openWorkspace}/>; if(route==='arcade')return <LandingPage onStart={openWorkspace} onNavigate={navigate} onOpenReconstruct={openReconstruct} onOpenEvidence={openEvidence}/>;
