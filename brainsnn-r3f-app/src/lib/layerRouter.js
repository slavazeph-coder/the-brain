...
export function runLayerRouter({ content, contentType = 'text', baseResult, providerTrace = [], engineStatus = {} } = {}) {
  ...
  const solitonField = computeSolitonField({ content: rawContent, firewallSignals, affectProfile });
  ...
  const engineTrace = [
    ...
    { stage: 'L103 39 Hz Soliton Field', status: 'completed', note: `Gamma coherence ${solitonField.gammaCoherence} at ${solitonField.effectiveFrequencyHz} Hz (${solitonField.synchrony}); ${solitonField.leapfrogEvents} leapfrog event(s).` },
