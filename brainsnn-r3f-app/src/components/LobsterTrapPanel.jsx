...
export default function LobsterTrapPanel() {
  ...
  const runTest = async () => {
    ...
    const { inspectPrompt } = await import("../utils/lobsterTrap");
    setTestResult(
      inspectPrompt({ prompt: testText, surface: "lobster.test", policy }),
    );
    setLog(loadLog());
  };
  ...
  return (
    <section className="panel panel-pad">
      ...
      <p className="muted">
        Every Gemini / Gemma call and every MCP tool dispatch is screened by
        Lobster Trap before it leaves the browser. Detects prompt injection,
        secret leakage, and PII; enforces policy on destructive agent tools.{" "}
        {remote
          ? `Remote endpoint: ${getRemoteUrl()}`
          : "Running in local-policy mode (set VITE_LOBSTER_TRAP_URL to use Veea remote inspection)."}
      </p>
      ...
      <div className="firewall-scores" style={{ marginTop: 12 }}>
        <div className="firewall-score-row">
          <div className="firewall-score-head">
            <span>Allowed</span>
            <strong style={{ color: ACTION_COLORS.allow }}>
              {stats.allow || 0}
            </strong>
          </div>
        </div>
        <div className="firewall-score-row">
          <div className="firewall-score-head">
            <span>Redacted</span>
            <strong style={{ color: ACTION_COLORS.redact }}>
              {stats.redact || 0}
            </strong>
          </div>
        </div>
        <div className="firewall-score-row">
          <div className="firewall-score-head">
            <span>Blocked</span>
            <strong style={{ color: ACTION_COLORS.block }}>
              {stats.block || 0}
            </strong>
          </div>
        </div>
      ...
      <div style={{ marginTop: 16 }}>
        ...
        >
          <input
            ...
            onChange={(e) =>
              update({ blockOnPromptInjection: e.target.checked })
            }
          />
          <span>
        ...
        >
          ...
          />
          <span>
            Allow destructive MCP tools (reset_brain, apply_scenario,
            trigger_burst)
          </span>
        </label>
        {remote && (
          ...
          >
            ...
            />
            <span>
              Use Veea remote inspection (falls back to local on error)
            </span>
          </label>
        )}
      </div>
      ...
      <div style={{ marginTop: 16 }}>
        ...
        {testResult && (
          <div className="firewall-result" style={{ marginTop: 12 }}>
            <div
              className="firewall-overall"
              style={{ "--risk": ACTION_COLORS[testResult.action] }}
            >
              <span>Decision</span>
              <strong style={{ color: ACTION_COLORS[testResult.action] }}>
            ...
            {testResult.redacted && (
              <div className="gemma-reasoning">
                <span className="eyebrow">
                  Redacted payload (this is what leaves the browser)
                </span>
      ...
      <div style={{ marginTop: 16 }}>
        ...
        {log.length === 0 ? (
          <p className="muted">
            No inspections yet. Trigger a Gemini scan, an MCP tool call, or use
            the tester above.
          </p>
        ) : (
          <ul
            className="firewall-evidence"
            style={{
              listStyle: "none",
              padding: 0,
              margin: "6px 0 0",
          ...
          >
            {log.slice(0, 20).map((e) => (
              ...
              >
                ...
                >
                  <strong style={{ color: ACTION_COLORS[e.action] }}>
