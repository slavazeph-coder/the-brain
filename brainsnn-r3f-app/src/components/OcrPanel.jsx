...
import SocialPostAutopsyPanel from './SocialPostAutopsyPanel';
 ...
 * /?scan=<text>.
 *
 * Layer 103 now sits directly below this panel so social screenshots can
 * flow from screenshot OCR into a full post/carousel autopsy.
 */
  ...
  async function handleFile(file) {
    ...
    try {
      ...
      try {
        ...
        setBoxes(found);
        setStatus(`OCR done — confidence ${Math.round(confidence)}% · ${words.length} words · ${found.length} annotations`);
      } catch {
        setBoxes([]);
        setStatus(`OCR done — confidence ${Math.round(confidence)}% · ${words.length} words · 0 annotations`);
      }
    } catch (e) {
