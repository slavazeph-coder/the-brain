...
export default function InspectorPanel({ state, onSelect }) {
  ...
  const incoming = LINKS
    .filter(([, to]) => to === selected)
    .map(([from, to]) => ({ id: `${from}\u2192${to}`, from, to, weight: state.weights[`${from}\u2192${to}`] }));

  const outgoing = LINKS
    .filter(([from]) => from === selected)
    .map(([from, to]) => ({ id: `${from}\u2192${to}`, from, to, weight: state.weights[`${from}\u2192${to}`] }));
    ...
    <aside className="inspector panel">
      <div className="panel-pad">
        <div className="inspector-head">
          ...
          <span className="region-pill" style={{ '--pill': info.color }}>
          ...
          {strongestOutgoing ? (
            <div className="link-highlight">
              ...
              <div className="weight-bar">
                <span style={{ width: `${strongestOutgoing.weight * 100}%` }} />
              </div>
            </div>
          ) : (
        ...
        <div className="subsection">
          ...
          <div className="region-list">
            {Object.entries(state.regions).map(([key, value]) => (
              <button
                key={key}
                className={`region-row ${key === selected ? 'active' : ''}`}
                onClick={() => onSelect(key)}
              >
                <span>{key}</span>
                <div className="region-row-right">
                  <div className="inline-bar">
                    <span style={{ width: `${value * 100}%`, background: REGION_INFO[key].color }} />
                  </div>
