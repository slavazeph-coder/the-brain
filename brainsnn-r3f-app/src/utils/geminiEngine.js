...
const API_BASE = import.meta.env.VITE_GEMINI_API_BASE || 'https://generativelanguage.googleapis.com/v1beta';
...
export async function captionWithGemini(file) {
  ...
  const raw = await callGemini(parts, 'You produce neutral, factual one-sentence image captions. Return ONLY JSON.');
  const cleaned = raw.replace(/```json\s*/gi, '').replace(/```\s*/gi, '').trim();
  return JSON.parse(cleaned).caption || '';
}

export async function rewriteWithGemini(text) {
  const raw = await callGemini([{ text }], REWRITE_SYSTEM_PROMPT, { temperature: 0.4 });
  const cleaned = raw.replace(/```json\s*/gi, '').replace(/```\s*/gi, '').trim();
...
export async function checkGeminiHealth() {
  ...
  try {
    const t0 = Date.now();
    const result = await analyzeContentWithGemini('This is a neutral test sentence with no manipulation.');
