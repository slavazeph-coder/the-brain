...
function loadCorpusFixtures() {
  try {
    const path = fileURLToPath(
      new URL("../../../hackathon/cache/corpus-fixtures.json", import.meta.url),
    );
...
const CORPUS_FIXTURES = loadCorpusFixtures();

function sha256Hex(text) {
  return createHash("sha256").update(text).digest("hex");
}

function expectedRuntimeCache(model = "gemini-2.5-flash") {
  const out = {};
  for (const [path, result] of Object.entries(expectedOutputs)) {
    if (path.startsWith("_")) continue;
    const body = CORPUS_FIXTURES[path];
    if (!body) continue;
    out[sha256Hex(`${model}:${body}`)] = result;
  }
  return out;
}
...
describe("firewallIntent — cache layer", () => {
  ...
  it("preloadIntentCache primes cache from disk", async () => {
    preloadIntentCache(expectedRuntimeCache());
