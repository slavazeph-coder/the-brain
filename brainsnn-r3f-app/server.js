...
app.get("/api/attacks", async (req, res) => {
  try {
    await handleAttacksGet(req, res);
  } catch (err) {
    console.error("[attacks:get]", err);
    res.status(500).json({ error: "attacks get failed" });
  }
});
app.post("/api/attacks", async (req, res) => {
  try {
    await handleAttackSubmit(req, res);
  } catch (err) {
    console.error("[attacks:post]", err);
    res.status(500).json({ error: "attacks post failed" });
  }
});
...
app.post("/api/score/stream", async (req, res) => {
  try {
    await handleScoreStream(req, res);
  } catch (err) {
...
app.post("/api/rooms", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "rooms post failed" });
  }
});
app.get("/api/rooms/:room", async (req, res) => {
  try {
    await handleRoomGet(req, res);
  } catch (err) {
    console.error("[rooms:get]", err);
    res.status(500).json({ error: "rooms get failed" });
  }
});
...
app.listen(PORT, "0.0.0.0", () => {
  ...
  if (!process.env.UPSTASH_REDIS_REST_URL) {
    console.log(
      "[brainsnn] UPSTASH_REDIS_REST_URL not set — leaderboard runs in-memory",
    );
  }
});
