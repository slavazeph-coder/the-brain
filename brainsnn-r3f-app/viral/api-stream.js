...
// Scorer + EN lexicons come from the shared core (see api-score.js) so the SSE
// stream's numbers match the on-page scan and /api/score exactly.
import { EN_RULES, scoreWithRules } from "../src/utils/firewallCore.js";

// The stream only emits the four dimensions; pull them off the shared scorer.
function scoreFull(text) {
  const s = scoreWithRules(text, EN_RULES);
  return {
    emotionalActivation: s.emotionalActivation,
    cognitiveSuppression: s.cognitiveSuppression,
    manipulationPressure: s.manipulationPressure,
    trustErosion: s.trustErosion,
  };
}
